...
def deinit():
    ...
    if g_debug:
        print "gpm_coop.py uninitialized"
...
def onGameStatusChanged(status):
    ...
    if status == bf2.GameStatus.Playing:
        ...
        g_controlPoints = bf2.objectManager.getObjectsOfType(
        ...
        for obj in g_controlPoints:
            ...
            if isHemi != 0:
                id = bf2.triggerManager.createHemiSphericalTrigger(
                    obj, onCPTrigger, "<<PCO>>", radius, (1, 2, 3)
                )
            else:
                id = bf2.triggerManager.createRadiusTrigger(
                    obj, onCPTrigger, "<<PCO>>", radius, (1, 2, 3)
                )
            obj.triggerId = id
        ...
        if g_debug:
            print "co-op gamemode initialized."
    else:
        bf2.triggerManager.destroyAllTriggers()
        g_controlPoints = []
...
def updateTicketLoss():
    ...
    ticketLossPerSecTeam1 = calcTicketLossForTeam(
        1, areaValueTeam2, -team1AreaOverweight
    )
    ticketLossPerSecTeam2 = calcTicketLossForTeam(
        2, areaValueTeam1, team1AreaOverweight
    )
...
def calcTicketLossForTeam(team, otherTeamAreaValue, otherTeamAreaOverweight):
    if otherTeamAreaValue >= 100 and otherTeamAreaOverweight > 0:
        ticketLossPerSecond = (
            bf2.gameLogic.getDefaultTicketLossPerMin(team) / 60.0
        ) * (otherTeamAreaOverweight / 100.0)
...
def onCPTrigger(triggerId, cp, vehicle, enter, userData):
    ...
    if vehicle and vehicle.getParent():
        return
        ...
        for p in playersInVehicle:
            ...
            if cp != None:
                if not p.getIsInsideCP():
                    if g_debug:
                        print "Resetting enterPctAt for player ", p.getName()
    ...
    if vehicle:
        for p in playersInVehicle:
            ...
            if p == playersInVehicle[0]:
                ...
                p.setIsInsideCP(0)
                if enter == 1:
                    bf2.gameLogic.sendHudEvent(
                        p, 66, 49
                    )  # 66 = HEEnableHelpMessage, 49 = VHMExitToCaptureFlag;
    ...
    for o in pcos:
        if not o:
            continue  # you can get None in the result tuple when the host can't figure out what object left the trigger
        if o.getParent():
            continue  # getOccupyingPlayers returns all players downwards in the hierarchy, so dont count them twice
    ...
    if team1Occupants == 0 and team2Occupants == 0:
        ...
        if cp.cp_getParam("flag") == attackingTeam or (
            cp.flagPosition == Bottom and cp.cp_getParam("team") == 0
        ):
    ...
    if (
        cp.cp_getParam("onlyTakeableByTeam") != 0
        and cp.cp_getParam("onlyTakeableByTeam") != attackingTeam
    ):
        return
    ...
    if (cp.flagPosition == Top and takeOverChangePerSecond > 0) or (
        cp.flagPosition == Bottom and takeOverChangePerSecond < 0
    ):
        takeOverChangePerSecond = 0.0
...
def onPlayerKilledCQ(victim, attacker, weapon, assists, object):
    ...
    if cp != None:
        ...
        if (
            attacker != None
            and attacker.getTeam() != victim.getTeam()
...
def getVectorDistance(pos1, pos2):
    ...
    return math.sqrt(
        diffVec[0] * diffVec[0] + diffVec[1] * diffVec[1] + diffVec[2] * diffVec[2]
    )
